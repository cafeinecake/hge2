...
BOOL CxImageWMF::CheckMetafileHeader(METAFILEHEADER *metafileheader)
{
  ...
  // check magic #
  if (metafileheader->key != 0x9ac6cdd7L) {
    return false;
  }
  ...
  // check resolution
  if ((metafileheader->inch <= 0) || (metafileheader->inch > 2540)) {
    return false;
  }
...
//  hMeta = SetWinMetaFileBits(len, p, NULL, NULL);
  ...
  mfp.xExt =
    10000; //(metafileheader->bbox.right - metafileheader->bbox.left) * cx1 / metafileheader->inch;
  mfp.yExt =
    10000; //(metafileheader->bbox.bottom - metafileheader->bbox.top) * cy1 / metafileheader->inch;
  mfp.hMF = 0;
