...
void CPSD::ProccessBuffer(unsigned char *pData)
{
  ...
  case 8: { // Duotone
    ...
    m_image.CreateFromArray(pRGBA, header_info.nWidth, header_info.nHeight, bAlpha ? 32 : 24,
                            header_info.nWidth * (bAlpha ? 4 : 3), true);
  ...
  case 2: { // Indexed
    if (!colour_mode_data.ColourData) {
      break;
    }

    if (colour_mode_data.nLength != 768) {
      break;
    }
    ...
    m_image.CreateFromArray(pRGB, header_info.nWidth, header_info.nHeight, 24, header_info.nWidth * 3,
                            true);
    delete [] pRGB;
  }
  break;
  ...
  case 3: { // RGB
    m_image.CreateFromArray(pData, header_info.nWidth, header_info.nHeight,
                            header_info.nChannels == 3 ? 24 : 32, header_info.nWidth * header_info.nChannels, true);
    m_image.SwapRGB2BGR();
  }
  break;
  ...
  case 7: { // Multichannel
    if (header_info.nChannels == 0 || header_info.nChannels > 4) {
      break;  // ???
    }
    ...
    for (int i = 0; i < nPixels; i++, pSrc += header_info.nChannels, pDst += 3) {
      ...
      if (header_info.nChannels > 2) {
        Y = (1.0 - (double)pSrc[2] / 256);
      }

      if (header_info.nChannels > 3) {
        K = (1.0 - (double)pSrc[3] / 256);
      }
...
{
  if (!ReadHeader(f, header_info)) {
    return -2;  // Error in header
  }

  if (!ReadColourModeData(f, colour_mode_data)) {
    return -3;  // Error in ColourMode Data
  }

  if (!ReadImageResource(f, image_resource)) {
    return -4;  // Error in Image Resource
  }

  if (!ReadLayerAndMaskInfoSection(f)) {
    return -5;  // Error in Mask Info
  }

  if (ReadImageData(f) != 0) {
    return -6;  // Error in Image Data
  }
  ...
  case 9: { // Lab
    ...
    for (int nChannel = 0; nChannel < header_info.nChannels; nChannel++) {
      ...
      for (int pos = 0; pos < nPixels;
           pos++, pDest += header_info.nChannels, pSource += bytesPerPixelPerChannel) {
        *pDest = *pSource;
      }
    }
...
{
  if (header_info.nBitsPerPixel != 8) {
    return -7;  // can't read this
  }
...
bool CxImagePSD::Decode(CxFile *hFile)
{
  ...
  cx_try {
    ...
    if (nErrorCode != 0)
    {
      cx_throw("error loading PSD file");
    }
    ...
    if (info.nEscape == -1 && info.dwType == CXIMAGE_FORMAT_PSD)
    {
      return true;
    }
