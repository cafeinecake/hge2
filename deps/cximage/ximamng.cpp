...
static mng_bool mymngreadstream(mng_handle mng, mng_ptr buffer, mng_uint32 size,
...
////////////////////////////////////////////////////////////////////////////////
static mng_bool mymngerror(mng_handle mng, mng_int32 code, mng_int8 severity, mng_chunkid chunktype,
                           mng_uint32 chunkseq, mng_int32 extra1, mng_int32 extra2, mng_pchar text)
{
...
CxImageMNG::~CxImageMNG()
{
  ...
  if (hmng) {
    mng_cleanup(&hmng);  //be sure it's not needed any more. (active timers ?)
  }
}
////////////////////////////////////////////////////////////////////////////////
...
bool CxImageMNG::Load(const TCHAR *imageFileName)
{
  ...
  if ((hFile = _tfopen(imageFileName, _T("rb"))) == nullptr) {
    return false;  // For UNICODE support
  }

#else

  if ((hFile = fopen(imageFileName, "rb")) == nullptr) {
    return false;
  }
  ...
  cx_try {
    ...
    if (mnginfo.animation_enabled == 0)
    {
      ...
      if (info.nFrame >= 0 && info.nFrame < info.nNumFrames) {
        for (int32_t n = 0; n < info.nFrame; n++) {
          mng_display_resume(hmng);
        }
      } else {
        cx_throw("Error: frame not present in MNG file");
      }
    }
...
bool CxImageMNG::Encode(CxFile *hFile)
{
  ...
  cx_try {
    if (head.biClrUsed != 0)
    {
      cx_throw("MNG encoder can save only RGB images");
    }
    ...
    if (mnginfo.image == nullptr)
    {
      cx_throw("could not allocate memory for MNG");
    }
