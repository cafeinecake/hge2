...
public:
  ...
  RGBQUAD GetPixelColorWithOverflow(int32_t x, int32_t y,
...
//@}
  ...
  int32_t Draw(HDC hdc, const RECT &rect, RECT *pClipRect = nullptr, bool bSmooth = false,
               bool bFlipY = false);
  ...
  int32_t DrawString(HDC hdc, int32_t x, int32_t y, const TCHAR *text, RGBQUAD color,
                     const TCHAR *font, int32_t lSize = 0, int32_t lWeight = 400, uint8_t bItalic = 0,
