...
bool CxImage::CreateFromHANDLE(HANDLE hMem)
{
  ...
  if (lpVoid) {
    ...
    if (dwCompression != BI_RGB || pHead->biBitCount == 32 || pHead->biBitCount == 16) {
      // get the bitmap bits
      LPSTR lpDIBBits = (LPSTR)((uint8_t *)pHead + * (uint32_t *)pHead + (uint16_t)(
                                  GetNumColors() * sizeof(
                                    RGBQUAD)));
...
bool CxImage::CreateFromHBITMAP(HBITMAP hbmp, HPALETTE hpal, bool bTransparency)
{
  ...
  if (hbmp) {
    ...
    if (bTransparency && IsHBITMAPAlphaValid(hbmp)) {
      ...
      if (dc) {
        if (GetDIBits(dc, hbmp, 0, bm.bmHeight, l_pRawBytes, &l_BitmapInfo, DIB_RGB_COLORS)) {
          l_bResult = CreateFromArray((uint8_t *)l_pRawBytes, bm.bmWidth, bm.bmHeight, bm.bmBitsPixel,
                                      bm.bmWidthBytes, false);
        } else {
          l_bResult = false;
        }
...
{
  return Draw(hdc, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, pClipRect,
              bSmooth, bFlipY);
}
////////////////////////////////////////////////////////////////////////////////
/**
 ...
 * \return true if everything is ok
 */
int32_t CxImage::Draw(HDC hdc, int32_t x, int32_t y, int32_t cx, int32_t cy, RECT *pClipRect,
                      bool bSmooth, bool bFlipY)
{
  if ((pDib == 0) || (hdc == 0) || (cx == 0) || (cy == 0) || (!info.bEnabled)) {
    return 0;
  }
  ...
  if (!(bTransparent || bAlpha || info.bAlphaPaletteEnabled)) {
    if (cx == head.biWidth && cy == head.biHeight) { //NORMAL
      ...
      if (pbase) {
        ...
        for (yy = 0; yy < desth; yy++) {
          ...
          for (xx = 0; xx < destw; xx++) {
            ...
            if (bSmooth) {
              if (fx > 1 && fy > 1) {
                c = GetAreaColorInterpolated(dx - 0.5f, dy - 0.5f, fx, fy, CxImage::IM_BILINEAR,
                                             CxImage::OM_REPEAT);
              } else {
      ...
      SetDIBitsToDevice(hdc, paintbox.left, paintbox.top, destw, desth, 0, 0, 0, desth, pbase, &bmInfo,
                        0);
 ...
 * \sa MakeBitmap
 */
HBITMAP CxImage::Draw2HBITMAP(HDC hdc, int32_t x, int32_t y, int32_t cx, int32_t cy,
                              RECT *pClipRect, bool bSmooth)
{
  if ((pDib == 0) || (hdc == 0) || (cx == 0) || (cy == 0) || (!info.bEnabled)) {
    return 0;
  }
  ...
  if (!bTransparent) {
    ...
    if (RectVisible(hdc, &rcDst)) {
      ...
      if (head.biBitCount < 24) {
        rgbBG = GetPaletteColor((uint8_t)info.nBkgndIndex);
      } else {
        rgbBG = info.nBkgndColor;
      }
 ...
 * \return true if everything is ok
 */
int32_t CxImage::Stretch(HDC hdc, int32_t xoffset, int32_t yoffset, int32_t xsize, int32_t ysize,
                         uint32_t dwRop)
{
  if ((pDib) && (hdc)) {
...
int32_t CxImage::DrawString(HDC hdc, int32_t x, int32_t y, const TCHAR *text, RGBQUAD color,
                            const TCHAR *font, int32_t lSize, int32_t lWeight, uint8_t bItalic, uint8_t bUnderline,
                            bool bSetAlpha)
...
{
  return LayerDrawAll(hdc, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top,
                      pClipRect, bSmooth);
}
////////////////////////////////////////////////////////////////////////////////
int32_t CxImage::LayerDrawAll(HDC hdc, int32_t x, int32_t y, int32_t cx, int32_t cy,
                              RECT *pClipRect, bool bSmooth)
{
  int32_t n = 0;
  CxImage *pLayer;
  ...
  while (pLayer = GetLayer(n++)) {
    if (pLayer->Draw(hdc, x + pLayer->info.xOffset, y + pLayer->info.yOffset, cx, cy, pClipRect,
                     bSmooth) == 0) {
      return 0;
    }

    if (pLayer->LayerDrawAll(hdc, x + pLayer->info.xOffset, y + pLayer->info.yOffset, cx, cy, pClipRect,
                             bSmooth) == 0) {
      return 0;
    }
  }
