  ...
  for (int32_t y = ymin; y < ymax; y++) {
    ...
    for (int32_t x = xmin; x < xmax; x++) {
...
#endif //CXIMAGE_SUPPORT_SELECTION
      {
        ...
        if (!bDirection && i < level) {
          BlindSetPixelColor(x, y, nBkgndColor, bSetAlpha);
        }
...
bool CxImage::SplitRGB(CxImage *r, CxImage *g, CxImage *b)
{
  ...
  for (int32_t y = 0; y < head.biHeight; y++) {
    for (int32_t x = 0; x < head.biWidth; x++) {
      ...
      if (r) {
        tmpr.BlindSetPixelIndex(x, y, color.rgbRed);
      }

      if (g) {
        tmpg.BlindSetPixelIndex(x, y, color.rgbGreen);
      }

      if (b) {
        tmpb.BlindSetPixelIndex(x, y, color.rgbBlue);
      }
    }
  }
...
bool CxImage::SplitCMYK(CxImage *c, CxImage *m, CxImage *y, CxImage *k)
{
  ...
  if (c == nullptr && m == nullptr && y == nullptr && k == nullptr) {
    return false;
  }
  ...
  for (int32_t yy = 0; yy < head.biHeight; yy++) {
    for (int32_t xx = 0; xx < head.biWidth; xx++) {
      ...
      if (c) {
        tmpc.BlindSetPixelIndex(xx, yy, (uint8_t)(255 - color.rgbRed));
      }

      if (m) {
        tmpm.BlindSetPixelIndex(xx, yy, (uint8_t)(255 - color.rgbGreen));
      }

      if (y) {
        tmpy.BlindSetPixelIndex(xx, yy, (uint8_t)(255 - color.rgbBlue));
      }

      if (k) {
        tmpk.BlindSetPixelIndex(xx, yy, (uint8_t)RGB2GRAY(color.rgbRed, color.rgbGreen, color.rgbBlue));
      }
    }
  }
...
bool CxImage::SplitYUV(CxImage *y, CxImage *u, CxImage *v)
{
  ...
  for (int32_t yy = 0; yy < head.biHeight; yy++) {
    for (int32_t x = 0; x < head.biWidth; x++) {
      ...
      if (y) {
        tmpy.BlindSetPixelIndex(x, yy, color.rgbRed);
      }

      if (u) {
        tmpu.BlindSetPixelIndex(x, yy, color.rgbGreen);
      }

      if (v) {
        tmpv.BlindSetPixelIndex(x, yy, color.rgbBlue);
      }
    }
  }
...
bool CxImage::SplitYIQ(CxImage *y, CxImage *i, CxImage *q)
{
  ...
  if (y == nullptr && i == nullptr && q == nullptr) {
    return false;
  }
  ...
  for (int32_t yy = 0; yy < head.biHeight; yy++) {
    for (int32_t x = 0; x < head.biWidth; x++) {
      ...
      if (i) {
        tmpi.BlindSetPixelIndex(x, yy, color.rgbGreen);
      }

      if (q) {
        tmpq.BlindSetPixelIndex(x, yy, color.rgbBlue);
      }
    }
  }
...
bool CxImage::SplitXYZ(CxImage *x, CxImage *y, CxImage *z)
{
  ...
  for (int32_t yy = 0; yy < head.biHeight; yy++) {
    for (int32_t xx = 0; xx < head.biWidth; xx++) {
      ...
      if (x) {
        tmpx.BlindSetPixelIndex(xx, yy, color.rgbRed);
      }

      if (y) {
        tmpy.BlindSetPixelIndex(xx, yy, color.rgbGreen);
      }

      if (z) {
        tmpz.BlindSetPixelIndex(xx, yy, color.rgbBlue);
      }
    }
  }
...
bool CxImage::SplitHSL(CxImage *h, CxImage *s, CxImage *l)
{
  ...
  for (int32_t y = 0; y < head.biHeight; y++) {
    for (int32_t x = 0; x < head.biWidth; x++) {
      ...
      if (h) {
        tmph.BlindSetPixelIndex(x, y, color.rgbRed);
      }

      if (s) {
        tmps.BlindSetPixelIndex(x, y, color.rgbGreen);
      }

      if (l) {
        tmpl.BlindSetPixelIndex(x, y, color.rgbBlue);
      }
    }
  }
...
RGBQUAD CxImage::RGBtoHSL(RGBQUAD lRGBColor)
{
  ...
  if (cMax == cMin) {   /* r=g=b --> achromatic case */
    ...
    if (L <= (HSLMAX / 2)) { /* saturation */
      ...
      S = (uint8_t)((((cMax - cMin) * HSLMAX) + ((2 * RGBMAX - cMax - cMin) / 2)) /
                    (2 * RGBMAX - cMax - cMin));
    }
 ...
 */
void CxImage::Mix(CxImage &imgsrc2, ImageOpType op, int32_t lXOffset, int32_t lYOffset,
                  bool bMixAlpha)
{
    ...
    for (int32_t lX = 0; lX < lWide; lX++) {
...
#endif //CXIMAGE_SUPPORT_SELECTION
      {
        ...
        case OpAdd:
          ...
          if (bEditAlpha) {
            rgbDest.rgbReserved = (uint8_t)max(0, min(255, rgb1.rgbReserved + rgb2.rgbReserved));
          }
        ...
        case OpAnd:
          ...
          if (bEditAlpha) {
            rgbDest.rgbReserved = (uint8_t)(rgb1.rgbReserved & rgb2.rgbReserved);
          }
        ...
        case OpScreen: {
          ...
          if (bEditAlpha) {
            rgbDest.rgbReserved = (uint8_t)((rgb1.rgbReserved * a) / 255);
          }
        }
        break;
                   ...
                   int32_t direction, bool bForceFFT, bool bMagnitude)
{
  ...
  if (srcReal == nullptr && srcImag == nullptr) {
    return false;
  }
  ...
  if (srcReal && dstReal) {
    tmpReal->Copy(*srcReal, true, false, false);
  }

  if (srcImag && dstImag) {
    tmpImag->Copy(*srcImag, true, false, false);
  }
  ...
  for (j = 0; j < h; j++) {
    for (k = 0; k < w; k++) {
      if (bMagnitude) {
        ...
        if (grid[k][j].x == 0) {
          tmpImag->SetPixelIndex(k, j, (uint8_t)max(0, min(255,
                                 (128 + (atan(grid[k][j].y / 0.0000000001)*nn)))));
        } else {
          tmpImag->SetPixelIndex(k, j, (uint8_t)max(0, min(255,
                                 (128 + (atan(grid[k][j].y / grid[k][j].x)*nn)))));
        }
      } else {
...
bool CxImage::Combine(CxImage *r, CxImage *g, CxImage *b, CxImage *a, int32_t colorspace)
{
  ...
  for (int32_t y = 0; y < h; y++) {
    ...
    for (int32_t x = 0; x < w; x++) {
      ...
      if (a) {
        AlphaSet(x, y, a->GetPixelIndex(x, y));
      }
...
bool CxImage::RepairChannel(CxImage *ch, float radius)
{
  ...
  for (x = 1; x < w; x++) {
    for (y = 1; y < h; y++) {
      ...
      correction = ((1.0 + iy * iy) * ixx - ix * iy * ixy + (1.0 + ix * ix) * iyy) /
                   (1.0 + ix * ix + iy * iy);
...
#endif //CXIMAGE_SUPPORT_SELECTION
      {
        ...
        for (j = -k2, i = 0; j < kmax; j++) {
          for (k = -k2; k < kmax; k++, i++) {
            ...
            if ((pix2.rgbBlue - pix1.rgbBlue) > maxb) {
              maxb = pix2.rgbBlue;
            }

            if ((pix2.rgbGreen - pix1.rgbGreen) > maxg) {
              maxg = pix2.rgbGreen;
            }

            if ((pix2.rgbRed - pix1.rgbRed) > maxr) {
              maxr = pix2.rgbRed;
            }
          }
        }
 ...
 * \author [DP]
 */
bool CxImage::TextBlur(uint8_t threshold, uint8_t decay, uint8_t max_depth, bool bBlurHorizontal,
                       bool bBlurVertical, CxImage *iDst)
{
  if (!pDib) {
    return false;
  }
    ...
    for (int32_t y = ymin; y < ymax; y++) {
      ...
      for (int32_t x = xmin; x < xmax; x++) {
...
#endif //CXIMAGE_SUPPORT_SELECTION
        {
          ...
          if (pLutA) {
            color.rgbReserved = pLutA[color.rgbReserved];
          }
...
#endif //CXIMAGE_SUPPORT_SELECTION
      {
        float a = 1.0f - 5.0f * ((float)((x - 0.5f * (xmax + xmin)) * (x - 0.5f * (xmax + xmin)) +
                                         (y - 0.5f * (ymax + ymin)) *
                                         (y - 0.5f * (ymax + ymin)))) / ((float)((xmax - xmin) * (ymax - ymin)));
...
bool CxImage::Solarize(uint8_t level, bool bLinkedChannels)
{
    ...
    if (IsGrayScale()) { //GRAYSCALE, selection
      ...
      for (uint32_t i = 0; i < head.biClrUsed; i++) {
        ...
        if (bLinkedChannels) {
          ...
          if (color.rgbBlue > level) {
            ppal[i].rgbBlue = (uint8_t)(255 - ppal[i].rgbBlue);
          }

          if (color.rgbGreen > level) {
            ppal[i].rgbGreen = (uint8_t)(255 - ppal[i].rgbGreen);
          }

          if (color.rgbRed > level) {
            ppal[i].rgbRed = (uint8_t)(255 - ppal[i].rgbRed);
          }
        }
      }
  ...
  if (pSelection == 0 || (!IsGrayScale() && IsIndexed())) {
    if (bLinkedChannels) {
      if ((uint8_t)RGB2GRAY(info.nBkgndColor.rgbRed, info.nBkgndColor.rgbGreen,
                            info.nBkgndColor.rgbBlue) > level) {
      ...
      if (info.nBkgndColor.rgbBlue > level) {
        info.nBkgndColor.rgbBlue = (uint8_t)(255 - info.nBkgndColor.rgbBlue);
      }

      if (info.nBkgndColor.rgbGreen > level) {
        info.nBkgndColor.rgbGreen = (uint8_t)(255 - info.nBkgndColor.rgbGreen);
      }

      if (info.nBkgndColor.rgbRed > level) {
        info.nBkgndColor.rgbRed = (uint8_t)(255 - info.nBkgndColor.rgbRed);
      }
    }
  }
...
int32_t  CxImage::OptimalThreshold(int32_t method, RECT *pBox, CxImage *pContrastMask)
{
  ...
  while (gray_min < 255 && p[gray_min] == 0) {
    gray_min++;
  }

  int32_t gray_max = 255;

  while (gray_max > 0 && p[gray_max] == 0) {
    gray_max--;
  }
  ...
  for (i = gray_min; i < gray_max; i++) {
    ...
    for (k = gray_min; k <= i; k++) if (p[k] > 0) {
        L -= p[k] * log(p[k] / w1) / w1;
      }

    for (k; k <= gray_max; k++) if (p[k] > 0) {
        L -= p[k] * log(p[k] / w2) / w2;
      }
 ...
 * \sa OptimalThreshold
 */
bool CxImage::AdaptiveThreshold(int32_t method, int32_t nBoxSize, CxImage *pContrastMask,
                                int32_t nBias, float fGlobalLocalBalance)
{
  if (!pDib) {
    return false;
  }
  ...
  if (fGlobalLocalBalance < 0.0f) {
    fGlobalLocalBalance = 0.0f;
  }

  if (fGlobalLocalBalance > 1.0f) {
    fGlobalLocalBalance = 1.0f;
  }
  ...
  for (int32_t y = 0; y < mh; y++) {
    for (int32_t x = 0; x < mw; x++) {
      ...
      mask.SetPixelIndex(x, y, (uint8_t)max(0, min(255,
                                            nBias + ((1.0f - fGlobalLocalBalance)*threshold + fGlobalLocalBalance * globalthreshold))));
    }
  }
 ...
 * \return true if everything is ok
 */
bool CxImage::FloodFill(const int32_t xStart, const int32_t yStart, const RGBQUAD cFillColor,
                        const uint8_t nTolerance,
