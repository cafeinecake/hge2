...
bool CxImageTGA::Decode(CxFile *hFile)
{
  ...
  cx_try {
    ...
    {
      hFile->Seek(tgaHead.IdLength,SEEK_CUR);  //skip descriptor
    }
    ...
    {
      AlphaCreate();  // Image has alpha channel
    }
    ...
    if (tgaHead.CmapType != 0)  // read the palette
    {
      ...
      for (int32_t i=0; i<tgaHead.CmapLength; i++) {
        SetPaletteColor((uint8_t)i,pal[i].b,pal[i].g,pal[i].r);
      }
    }
    ...
    for (int32_t y=0; y < tgaHead.ImageHeight; y++)
    {
      ...
      if (bYReversed) {
        pDest = iter.GetRow(tgaHead.ImageHeight-y-1);
      } else {
        pDest = iter.GetRow(y);
      }
    ...
    if (bYReversed && tgaHead.PixelDepth==32)
    {
      AlphaFlip();  //<lioucr>
    }
...
////////////////////////////////////////////////////////////////////////////////
uint8_t CxImageTGA::ExpandCompressedLine(uint8_t* pDest,TGAHEADER* ptgaHead,CxFile *hFile,
    int32_t width, int32_t y, uint8_t rleLeftover)
{
  uint8_t rle;
  int32_t filePos=0;
...
////////////////////////////////////////////////////////////////////////////////
void CxImageTGA::ExpandUncompressedLine(uint8_t* pDest,TGAHEADER* ptgaHead,CxFile *hFile,
                                        int32_t width, int32_t y, int32_t xoffset)
{
  switch (ptgaHead->PixelDepth) {
  case 8:
