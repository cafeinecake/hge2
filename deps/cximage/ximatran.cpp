  ...
  bool const disableAveraging)
{
  ...
  if ((xScale <= 1 && yScale <= 1) || disableAveraging) {
    ...
    for (dY = 0; dY < newy; dY++) {
      ...
      for (dX = 0; dX < newx; dX++) {
        sX = (dX + 0.5f) * xScale - 0.5f;
        newImage.SetPixelColor(dX, dY, GetAreaColorInterpolated(sX, sY, xScale, yScale, inMethod, ofMethod,
                               0),
                               true);
      }//for x
    }//for y
...
bool CxImage::QIShrink(int32_t newx, int32_t newy, CxImage *const iDst, bool bChangeBpp)
{
  ...
  uint32_t *accu = new
  uint32_t[newx *
           accuCellSize];    //array for suming pixels... one pixel for every destination column
