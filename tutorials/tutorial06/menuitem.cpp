...
hgeGUIMenuItem::hgeGUIMenuItem(int _id, hgeFont *_fnt, HEFFECT _snd, float _x, float _y,
                               float _delay, const char *_title)
{
  float w;
...
{
  if (hgeut::flt_not_equal(timer2, -1.0f)) {
    timer2 += dt;
  ...
  } else if (hgeut::flt_not_equal(timer, -1.0f)) {
    timer += dt;
