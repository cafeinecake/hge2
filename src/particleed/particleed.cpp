...
void InitEditor()
{
  ...
  if (bgName) {
    ...
    state.sprBG = new hgeSprite(state.texBG, 0, 0,
                                static_cast<float>(bgw),
...
void CreateGUI()
{
  ...
  slider->SetMode(0, 2 * static_cast<float>(M_PI), HGESLIDER_BAR);
  slider->SetValue(static_cast<float>(M_PI));
  gui->AddCtrl(slider);
  ...
  slider->SetMode(0, 2.f * static_cast<float>(M_PI), HGESLIDER_BAR);
  slider->SetValue(static_cast<float>(M_PI));
  gui->AddCtrl(slider);
