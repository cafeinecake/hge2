...
{
  if(!handle) {
    handle=(size_t)hge->Resource_Load(filename);
  }
...
{
  if(!handle) {
    handle=(DWORD)hge->Texture_Load(filename, 0, mipmap);
  }
...
{
  if(!handle) {
    handle=(DWORD)hge->Effect_Load(filename);
  }
...
{
  if(!handle) {
    handle=(DWORD)hge->Stream_Load(filename);
  }
...
{
  if(!handle) {
    handle=(DWORD)hge->Target_Create(width, height, zbuffer);
  }
...
{
  if(!handle) {
    handle = (size_t)new hgeStringTable(filename);
  }
